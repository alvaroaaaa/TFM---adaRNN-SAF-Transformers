...
def create_sequences(features, labels, sequence_length):
    """
    Create sequences of data points based on the specified sequence length.
    """
...
def create_loader(features, labels, batch_size, sequence_length):
    """
    Create DataLoader from features and labels with sequences.
    """
    ...
    labels_tensor = torch.tensor(sequence_labels, dtype=torch.float32)  # Add dimension for labels

    dataset = TensorDataset(features_tensor, labels_tensor)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)  # Consider shuffle=True if appropriate
    return loader
